  ...
  _run: function runTask(passedOptions) {
    ...
    return this._getNpmDependenciesPaths()
      ...
      .then(function(dependencies) {
        ...
        var foundDeps = [];
        // deps of foundDeps in given step, which are listed in 'dependencies' input
        var processedDeps = [];
        // all deps, which were already processed in any step
        var finishedDeps = [];
        // deps of 'dependencies', which weren't put to finishedDeps
        var remainingDeps = dependencies.slice();
        // for storing indexes of found elements in lists
        var position = -1;

        // whole lines of logBuffer (ending '\n')
        var finishedLines = [];
        ...
        installation.progress(function(childProcess) {
          childProcess.stderr.on('data', function(data) {
            ...
            /**
             * Add those deps, which are listed in 'dependencies', to empty processedDeps for this step,
             * and remove them from remainingDeps to prevent duplicities and optimize the searching times
             */
            foundDeps.forEach(function(dep) {
              if (~(position = remainingDeps.indexOf(dep))) {
                processedDeps.push(remainingDeps.splice(position, 1));
              }
            });
            ...
            if (processedDeps) {
              ...
              // empty processedDeps of this step
